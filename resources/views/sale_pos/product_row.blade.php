...
<tr class="product_row" data-row_index="{{ $row_count }}" @if (!empty($so_line)) data-so_id="{{ $so_line->transaction_id }}" @endif>
        ...
        @if (in_array('modifiers', $enabled_modules))
            <div class="modifiers_html">
                @if (!empty($product->product_ms))
                    @include('restaurant.product_modifier_set.modifier_for_product', array('edit_modifiers' => true,
                    'row_count' => $loop->index, 'product_ms' => $product->product_ms ) )
                @endif
            </div>
        @endif
                ...
                <select class="form-control lot_number input-sm" name="products[{{ $row_count }}][lot_no_line_id]"
                    ...
                    @foreach ($product->lot_numbers as $lot_number)
                        ...
                        <option value="{{ $lot_number->purchase_line_id }}"
                            ...
                            {{ $selected }}>@if (!empty($lot_number->lot_number) && $lot_enabled == 1){{ $lot_number->lot_number }} @endif @if ($lot_enabled == 1 && $exp_enabled == 1) - @endif @if ($exp_enabled == 1 && !empty($lot_number->exp_date)) @lang('product.exp_date'): {{ @format_date($lot_number->exp_date) }} @endif {{ $expiry_text }}</option>
                    @endforeach
                </select>
            @endif
        @endif
    ...
    @if (!empty($is_direct_sell))
        ...
        @php
            ...
            $custom_profit_percent = $product->custom_profit_percent;
            if (!empty($so_line)) {
	...
	@can('purchase.view')
        <td @if (!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
            <input type="text" name="products[{{ $row_count }}][last_purchased_price]"
                class="form-control pos_purchase_price input_number mousetrap"
                value="{{ @num_format($purchase_price) }}" disabled>
        </td>
		@endcan
		@can('purchase.view')
        <td @if (!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
            <input type="text" name="products[{{ $row_count }}][profit_percent]"
                class="form-control pos_profit_percent input_number mousetrap"
                value="{{ @num_format($profit_percent) }}">
            <br>
            <label for="custom_profit_percent">Custom Profit Percent</label>
            <input type="text" name="products[{{ $row_count }}][custom_profit_percent]"
                class="form-control custom_profit_percent input_number mousetrap"
                value="{{ @num_format($custom_profit_percent) }}">
        </td>
		@endcan
        <td @if (!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
            <input type="text" name="products[{{ $row_count }}][unit_price]"
                class="form-control pos_unit_price input_number mousetrap" value="{{ @num_format($pos_unit_price) }}">
        </td>
    ...
    <td class="{{ $hide_tax }}">
        <input type="text" name="products[{{ $row_count }}][unit_price_inc_tax]"
            ...
            @if (!$edit_price) readonly @endif @if (!empty($pos_settings['enable_msp'])) data-rule-min-value="{{ $unit_price_inc_tax }}" data-msg-min-value="{{ __('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax)]) }}" @endif>
    </td>
    ...
    <td class="text-center">
        ...
        @endphp
        <input type="{{ $subtotal_type }}" class="form-control pos_line_total @if (!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif"
