            ...
            if (auth()->user()->can('user.view') || auth()->user()->can('user.create') || auth()->user()->can('roles.view')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('roles.view')) {
                            $sub->url(
                                action('AreaController@index'),
                                __('Area'),
                                ['icon' => 'fa fas fa-briefcase', 'active' => request()->segment(1) == 'area']
                            );
                        }

                        if (auth()->user()->can('roles.view')) {
                            $sub->url(
                                action('SellTargetController@index'),
                                __('Sales Target'),
                                ['icon' => 'fa fas fa-briefcase', 'active' => request()->segment(1) == 'sale-target']
                            );
                        }
                    },
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.opening_stock')) {
                            $sub->url(
                                action('OpeningStockController@index'),
                                __('Opening Stock List'),
                                ['icon' => 'fa fas fa-download', 'active' => request()->segment(1) == 'opening-stock-list']
                            );
                        }
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            ...
                            $sub->url(
                             ...
                             $sub->url(
                                action('ReportController@getTargetAchievementReport'),
                                __('Target Achievement Report'),
                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'target-achievement-report']
                            );

                            $sub->url(
                                action('ReportController@getproductQuotationReport'),
                                __('lang_v1.product_quotation_report'),
                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'product-quotation-report']
                            );
