...
class ReportController extends Controller
{
    ...
    public function getTargetAchievementReport(Request $request)
    {

        $business_id = $request->session()->get('user.business_id');
        $mpo_users = User::role('MPO#' . $business_id)->get();
        if ($request->ajax()) {
            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            // $customerId = $_GET['customerID'];
            $credit_customer_sales = array();
            $output = array();

            // $mpo_users = User::role('MPO'.'#'.$business_id)->get();
            foreach ($mpo_users as $user) {
                $output[$user->id]['user_id'] = 0;
                $output[$user->id]['name'] = 0;
                $output[$user->id]['target'] = 0;
            }

            foreach ($mpo_users as $user) {
                $output[$user->id]['user_id'] = $user->id;
                $output[$user->id]['name'] = $user->first_name;
                // $points = RewardPoints::leftjoin('users as cp','cp.id','=','customer_id')
                if (!empty($start_date) && !empty($end_date)) {
                    $targets = SellTarget::where('user_id', $user->id)
                        ->where('business_id', $business_id)
                        ->whereBetween(DB::raw('date(sell_targets.updated_at)'), [$start_date, $end_date])->get();
                }
                ...
                $output[$user->id]['target'] = $totalTarget;

                // if (!empty($start_date) && !empty($end_date)) {
                //     $all_transaction_returns = DB::table('transactions as ts')
                //         ->select('ts.id', 'ts.final_total')
                //         ->where('ts.status', 'draft')
                //         ->where('ts.type', 'sell')
                //         ->where('ts.is_quotation',1)
                //         ->where('ts.created_by', $user->id)
                //         ->whereBetween(DB::raw('date(ts.transaction_date)'), [$start_date, $end_date])
                //         ->get();
                //     $output[$user->id]['total_quotation'] = 0;
                //     foreach ($all_transaction_returns as $tr) {
                //         $output[$user->id]['total_quotation'] += $tr->final_total;
                //     }
                // } else {
                //     $all_transaction_returns = DB::table('transactions as ts')
                //         ->select('ts.id', 'ts.final_total')
                ...
                $sales = Transaction::where('transactions.created_by', $user->id)
                    ->where('transactions.type', 'sell')
                    ->where('transactions.status', 'draft')
                    ->where('transactions.is_quotation', 1)
                    // ->where('ts.method', 'credit')
                    ->select(
                        'transactions.id',
                        'final_total'
                    )
                ...
                //     $mpo_id = $request->get('mpo_id', null);
                // if (!empty($customer_id)) {
                //     $sales->where('transactions.created_by', $mpo_id);
                // }

                $all_sales = $sales->get();
                $output[$user->id]['total_sale'] = 0;
                $output[$user->id]['achievement'] = 0;
                $output[$user->id]['percentage'] = 0;
                foreach ($all_sales as $sale) {
                    $output[$user->id]['total_sale'] += $sale->final_total;
                }
                $output[$user->id]['total_sale'] = $output[$user->id]['total_sale'];
                $output[$user->id]['difference'] = $output[$user->id]['target'] - $output[$user->id]['total_sale'];

                if ($output[$user->id]['target'] !== 0 && $output[$user->id]['total_sale']) {
                    $output[$user->id]['percentage'] = $output[$user->id]['total_sale'] / $output[$user->id]['target'] * 100;
                    $output[$user->id]['achievement'] =number_format((float)$output[$user->id]['percentage'], 2, '.', '').'%';
                }
            }

            return Datatables::of($output)
                ->rawColumns(['name', 'target', 'total_sale', 'difference', 'achievement'])
                ->make(true);
        }
        $business_locations = BusinessLocation::forDropdown($business_id);
        // $customers = Contact::customersDropdown($business_id);
        $mpo_users = User::role('MPO#' . $business_id)->pluck('first_name', 'id')->toArray();

        return view('report.target_achievement_report')
            ->with(compact('business_locations', 'mpo_users'));
    }

    /**
     * Shows product quotation report
     *
     * @return \Illuminate\Http\Response
     */
    public function getproductQuotationReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $business_locations = BusinessLocation::forDropdown($business_id);
        $customers = Contact::customersDropdown($business_id);
        $areas = Area::where('business_id', $business_id)->pluck('name', 'id');
        $users = User::role('MPO' . '#' . $business_id)->pluck('first_name', 'id');
        return view('report.product_quotation_report')
            ->with(compact('business_locations', 'customers', 'areas', 'users'));
    }
